  ...
  return (
    <div style={
      ...
      <div onClick={() => setHeight(
        height === 30 ? items.length * 42 + 30 : 30
      )} className="flex justify-between items-center">
        <p className="text-textPrimary font-bold text-base leading-[18px]">{title}</p>
      ...
      {items && (
        <div className="flex flex-col gap-6 mt-6">
          {items.map((item: MenuItem, index: number) => (
            <a key={index} href={item.url} className="text-textPrimary text-xs leading-[18px]">
              {item.label}
            </a>
          ))}
        </div>

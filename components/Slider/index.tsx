...
const Slider = ({ children, spaceBetween, slidesPerView, dots, sliderName, breakpoints, arrow }: SliderProps) => {
  useLayoutEffect(() => {
    ...
    swiper.on('slideChange', () => {
      const dots = document.querySelectorAll(`.dots-${sliderName} .swiper-pagination-bullet`);
      ...
      </div>
      {
        arrow && (
          <div className={`${sliderName === "shelf" ? "px-0 !hidden lg:!flex" : "px-7 !flex"} container absolute top-0 left-0 right-0  h-full items-center justify-between`}>
            ...
            <div className={`swiper-button-next swiper-button-next-${sliderName} !static`}>
              <Image src="/arrow-right.svg" alt="flecha para dirteita" width={20} height={32} />
            </div>
          </div>
        )
      }
